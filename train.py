...
if __name__ == '__main__':
    ...
    model = LSTMMultiBinaryClassificationNet(num_embeddings=len(ds_train.vocab), embedding_dim=128, num_classes=6,
                                             padding_idx=padding_idx, lstm_layers=2, hidden_size=512, p_dropout=0.5,
                                             additional_fc_layer=128, dev=device)
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for j, ((tokens, targets), (input_lengths, _)) in enumerate(train_loader):
            ...
            loss = -1/len(targets) * (positive_class_weights*targets*log(output) + (1 - targets)*log(1 - output)).sum()
